...
export function deconstructHirePurchaseInvoiceAttestationData(recipients: string[], vin: string, invoiceID: string, amount: number, due: Date, linkedAttestationId: string ) {
    const schemaData = {
        vin: vin,
        invoiceID: invoiceID,
        amount: amount,
